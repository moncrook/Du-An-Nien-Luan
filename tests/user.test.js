...
// Giả lập (Mock) hoàn toàn module database
...
describe('Test các API xác thực Người dùng (User APIs)', () => {
  ...
  test('POST /api/register - Nên đăng ký tài khoản thành công', async () => {
    // Giả lập hành vi: findUser trả về null (user chưa tồn tại)
    db.queryOne.mockResolvedValue(null); 
    
    // Giả lập hành vi: Insert thành công và Query lại User thành công
    db.querySql.mockResolvedValueOnce([{ affectedRows: 1 }]); 
    db.querySql.mockResolvedValueOnce([{ id: 1, username: 'testuser', nickname: 'Test' }]); 
